...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!state || !storedState || state !== storedState) {
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/auth/login?error=invalid_state`)
    }

    if (!codeVerifier) {
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/auth/login?error=missing_verifier`)
    }
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/auth/login?error=auth_failed`)
  }
}
