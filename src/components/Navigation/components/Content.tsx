...
export const Content: React.FC<Props> = ({ onOpenFilters, activeFiltersCount = 0 }) => {
    ...
    <div className="relative">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4 lg:space-x-6 flex-1">
            ...
            <div className="hidden lg:flex">
              ...
              >
                ...
                <span className="font-medium text-sm xl:text-base">Filtrar productos</span>
            ...
            <div className="hidden md:flex space-x-1 lg:space-x-2">
                  ...
                  >
                    <Link href={item.href}>
                      ...
                      <span className="text-xs lg:text-sm font-medium text-center leading-tight">
                        {window.innerWidth < 1024 ? item.shortLabel : item.label}
                      </span>
              ...
              {isSearchPage && (
                <div className="flex flex-col h-auto py-2 lg:py-3 px-2 lg:px-4 gap-1 min-w-[90px] lg:min-w-[120px] bg-primary-container text-on-primary-container shadow-md border border-primary/20 rounded-xl">
                  ...
                  <span className="text-xs lg:text-sm font-medium text-center leading-tight">
                    {searchQuery && searchQuery.length > 10 ? `"${searchQuery.substring(0, 10)}..."` : searchQuery ? `"${searchQuery}"` : 'Búsqueda'}
                  </span>
                  <span className="text-xs text-on-surface-variant/70 hidden lg:block leading-tight text-center">
                    Productos filtrados
                  </span>
            ...
            {(pathname.includes('/product/') || isCollectionPage) && (
              ...
              >
                <Link href="/store">
                  ...
                  <span className="font-medium text-sm lg:text-base">Volver a la tienda</span>
                </Link>
              </Button>
            )}
          </div>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden ">
          <div className="fixed inset-0 bg-surface-variant" onClick={toggleMobileMenu} />
                  ...
                  >
                    <Link href={item.href}>
                      ...
                      <div className="flex flex-col items-start">
                        ...
                        <span className="text-xs text-on-surface-variant/70">{item.description}</span>
                      </div>
                    </Link>
                  </Button>
                );
          ...
          <div className="container mx-auto px-3 sm:px-4 lg:px-6 py-2 lg:py-3">
            <div className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm overflow-x-auto scrollbar-hide">
              ...
              {pathname.includes('/product/') && (
                <>
                  <ChevronRight className="w-3 h-3 text-on-surface-variant/50 shrink-0" />
                  <span className="text-on-surface font-medium px-1 sm:px-2 shrink-0">Producto</span>
                </>
              )}
            </div>
          </div>
