...
export function CouponCreatorModal({
  ...
  const form = useForm<CouponFormData>({
    defaultValues: {
      ...
      appliesTo: isEditMode
        ? couponToEdit.appliesTo
    ...
    try {
      ...
      if (isEditMode && onCouponUpdated) {
        ...
        const couponData: CreateDiscountInput = {
          ...
          endsAt: data.endsAt?.toISOString(),
          productIds:
            data.appliesTo === 'SPECIFIC_PRODUCTS' ? selectedProducts.map((p) => p.id) : undefined,
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : `Error al ${isEditMode ? 'actualizar' : 'crear'} el cupón`
      )
    } finally {
      setIsCreating(false)
    }
                      ...
                      <div className='flex justify-between'>
                        ...
                        <span>
                          {watchedAppliesTo === 'ALL_PRODUCTS'
                            ? 'Todos'
                            : `${selectedProducts.length} específicos`}
                        </span>
                      </div>
                    </div>
                  </div>
