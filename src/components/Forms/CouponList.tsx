  ...
  return (
    <div className='space-y-4'>
          ...
          <Card key={coupon.id} className='transition-shadow hover:shadow-md'>
              ...
              <div className='flex items-start justify-between'>
                <div className='flex items-center gap-2'>
                  ...
                  <div>
                    ...
                    <p className='text-sm text-muted-foreground'>{getAppliesToLabel(coupon)}</p>
                  </div>
                </div>
                <div className='flex items-center gap-2'>
            ...
            <CardContent className='space-y-3'>
              ...
              <div className='flex items-center justify-between'>
                ...
                <span className='text-sm text-muted-foreground'>{getAppliesToLabel(coupon)}</span>
              </div>

              <div className='flex items-center justify-between'>
                <span className='text-sm font-medium'>Inicio:</span>
                <span className='text-sm text-muted-foreground'>{formatDate(coupon.startsAt)}</span>
              </div>

              {coupon.endsAt && (
                <div className='flex items-center justify-between'>
                  <span className='text-sm font-medium'>Fin:</span>
                  <span className='text-sm text-muted-foreground'>{formatDate(coupon.endsAt)}</span>
                </div>
              )}
      ...
      <CouponCreatorModal
        ...
        selectedProducts={
          editingCoupon?.productIds?.map((id) => ({
            id,
            title: `Producto ${id.split('/').pop()}`,
          })) ?? []
        }
        mode='edit'
        ...
        <DialogContent className='max-w-md'>
          ...
          <div className='space-y-4'>
            <p className='text-muted-foreground'>
              ¿Estás seguro de que quieres eliminar el cupón <strong>{couponToDelete?.code}</strong>
              ?
            </p>
