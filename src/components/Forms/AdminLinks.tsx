...
interface AdminLinksProps {
  ...
  onCreateLink: (data: { platform: string; url: string }) => Promise<void>
  onUpdateLink: (linkId: string, data: { platform: string; url: string }) => Promise<void>
  onDeleteLink: (linkId: string) => Promise<void>
  onUpdateLinksOrder: (data: { id: string; order: number }[]) => Promise<void>
}
...
export const AdminLinks = ({
  ...
  const handleAddLink = async () => {
    ...
    await onCreateLink({ platform: selectedPlatform, url: finalUrl })
    setNewLinkUrl('')
  }

  const handleDeleteLink = async (linkId: string) => await onDeleteLink(linkId)
  ...
  const handleSaveEdit = async (linkId: string) => {
    ...
    await onUpdateLink(linkId, { platform: platform?.id || 'custom', url: finalUrl })
    handleCancelEdit()
  }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      onUpdateLinksOrder(reorderPayload)
    }
  }
    ...
    <Card className='border-none shadow-none'>
      <CardHeader className='px-0 pt-0'>
        <CardTitle className='text-lg font-semibold text-foreground'>
          Administrar enlaces del usuario
        </CardTitle>
      </CardHeader>
      <CardContent className='space-y-6 px-0'>
        <div className='space-y-2 rounded-lg border border-outline p-4'>
          ...
          <div className='flex flex-col gap-2 sm:flex-row'>
            ...
            >
              <PlusCircle className='mr-2 size-4' /> {isCreating ? 'Añadiendo...' : 'Añadir'}
            </Button>
          </div>
        </div>

        <div className='space-y-3'>
          <h3 className='font-medium text-muted-foreground'>Enlaces del usuario</h3>
          {isLoading ? (
            ...
            <div className='rounded-md bg-surface-container-low p-6 text-center text-muted-foreground'>
              <p>El usuario aún no ha añadido ningún enlace.</p>
            </div>
          )}
        </div>
      </CardContent>
